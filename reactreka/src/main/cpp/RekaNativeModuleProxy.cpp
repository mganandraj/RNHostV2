...
/*static */ local_ref<RekaNativeModuleProxy::jhybriddata> RekaNativeModuleProxy::initHybrid(alias_ref<jclass>, alias_ref<ReactContextHolder::javaobject> reactContextHolder) {
    return makeCxxInstance(reactContextHolder);
}

/*static*/ void RekaNativeModuleProxy::registerNatives() {
    registerHybrid({
        makeNativeMethod("initHybrid", RekaNativeModuleProxy::initHybrid),
        makeNativeMethod("getConstantsImpl", RekaNativeModuleProxy::getConstantsImpl),
        makeNativeMethod("invokeNativeImpl", RekaNativeModuleProxy::invokeNativeImpl)
    });
}

RekaNativeModuleProxy::RekaNativeModuleProxy(alias_ref<ReactContextHolder::javaobject> reactContextHolder) {
    ...
    rekaOptions.RekaServiceProviderFactory =
        [dataServiceProviderName = std::move(dataServiceProviderName)](Mso::JSHost::IRekaContext & context) mutable noexcept
    {
        return Mso::JSHost::GetOrMakeRekaServiceProvider(dataServiceProviderName.c_str(), context);
    };

    rekaOptions.OnInitialized = [](Mso::JSHost::IRekaContext &) noexcept
    {};

    rekaOptions.JsExecutor = Mso::Make<ReactNativeRekaBridge, Mso::JSHost::IRekaBridge>(reactContextHolder);
...
local_ref<ReadableNativeMap::jhybridobject> RekaNativeModuleProxy::getConstantsImpl() {
    ...
    m_rekaInstance->EnumerateConstants([&constantValues](const char* serviceName, const char* propertyName, const char* serializedJson) noexcept {
        constantValues
...
local_ref<RekaNativeToJsBridge::JavaPart> RekaNativeToJsBridge::create(alias_ref<ReactContextHolder::javaobject> contextHolder) {
    return newObjectJavaArgs(contextHolder);
}

ReactNativeRekaBridge::ReactNativeRekaBridge(alias_ref<ReactContextHolder::javaobject> reactContextHolder) noexcept {
...
void ReactNativeRekaBridge::Invoke(const Mso::JSHost::RekaBridgeInvokeArgs& args) noexcept {
    ...
    local_ref<ReadableNativeMap::jhybridobject> map =  ReadableNativeMap::newObjectCxxArgs(std::move(arg));
...
void RekaNativeToJsBridge::callJSFunction(const std::string& methodName, local_ref<ReadableNativeMap::jhybridobject> args) {
    static const auto method = javaClassLocal()->getMethod<void(jstring,facebook::react::ReadableNativeMap::javaobject)>("CallJSFunction");
    local_ref<JString> jMethodName = make_jstring(methodName);
    method(m_javaPart.get(), jMethodName.get(), args.get());
}

/*static*/ local_ref<RekaNativeToJsBridge::jhybriddata> RekaNativeToJsBridge::initHybrid(alias_ref<jhybridobject> jThis){
