...
LIBLET_PUBLICAPI JSBundleRegistration::JSBundleRegistration(const char* jsBundleId, const char* jsBundleFileName) noexcept
	: m_jsBundleId { jsBundleId }
	, m_jsBundleFileName { jsBundleFileName }
	, m_next { g_jsBundleRegistration }
{
...
LIBLET_PUBLICAPI std::vector<Mso::CntPtr<IJSBundle>> GetRegisteredJSBundles(std::initializer_list<std::string_view> jsBundleIds) noexcept
{
...
LIBLET_PUBLICAPI void AddRegisteredJSBundles(std::vector<Mso::CntPtr<IJSBundle>>& jsBundles, std::initializer_list<std::string_view> jsBundleIds) noexcept
{
	...
	for (auto ch : jsonValue)
	{
		if (ch < 127) // ASCII characters
		{
			if (ch >= ' ') // non-control character - escape \ and "
			{
				++escapedSize;
				if (ch == '\\') ++escapedSize;
				else if (ch == '"') ++escapedSize;
			}
			else // a control character - escape most common ones and skip others
			{
				if (ch == '\r') escapedSize += 2;
				else if (ch == '\n') escapedSize += 2;
				else if (ch == '\t') escapedSize += 2;
				else if (ch == '\b') escapedSize += 2;
				else if (ch == '\v') escapedSize += 2;
				else if (ch == '\f') escapedSize += 2;
			}
		}
	}
	...
	for (auto ch : jsonValue)
	{
		if (ch < 127) // ASCII characters
		{
			...
			else // a control character - escape most common ones and skip others
			{
				...
				else if (ch == '\t') *ptr++ = '\\', *ptr++ = 't';
				else if (ch == '\b') *ptr++ = '\\', *ptr++ = 'b';
				else if (ch == '\v') *ptr++ = '\\', *ptr++ = 'v';
				else if (ch == '\f') *ptr++ = '\\', *ptr++ = 'f';
			}
		}
	}
	*ptr++ = '"'; // string end
...
{
	DynamicJSBundle(std::string&& id, std::string&& value) noexcept
		: m_id { std::move(id) }
