...
template <class T>
CntPtr<T> MakeTCntPtr(T* ptr) noexcept
{
	return CntPtr<T>{ptr};
}
...
{
	InvokeElsePostExecutor(Mso::DispatchQueue queue) noexcept
		: m_queue{queue}
	{
	}
	...
	{
		m_queue.InvokeElsePost(std::move(callback));
	}
